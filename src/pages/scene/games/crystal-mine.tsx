...
export default function CrystalMineGame({source,onComplete}:{source:string,onComplete:(m:string,n:string)=>void}){
  ...
  if(phase === 'game' && gamePicked){
    if(gamePicked.id === 'memory') return <MemoryMatch source={source} onEnd={onGameEnd} accentColor={cfg.accentColor} bgColor={cfg.bgColor} />
    if(gamePicked.id === 'match3') return <MatchThree source={source} onEnd={onGameEnd} accentColor={cfg.accentColor} bgColor={cfg.bgColor} />
    if(gamePicked.id === 'stack') return <StackMatch onEnd={onGameEnd} accentColor={cfg.accentColor} bgColor={cfg.bgColor} />
    return <div style={{padding:40,alignItems:'center',paddingTop:100}}><span style={{color:'#fff',fontSize:14}}>未知游戏</span></div>
  }

  return(
    <div style={{minHeight:'100vh',background:'#1a1020',overflow:'hidden',display:'flex',flexDirection:'column'}}>
      <style>{KF}</style>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',
        ...
        <div style={{flex:1,
          ...
          <span style={{fontSize:20,fontWeight:700,color:'#fff',textShadow:'0 2px 8px rgba(0,0,0,0.5)',marginBottom:4}}>{cfg.name}</span>
          <span style={{fontSize:10,color:'rgba(255,255,255,0.4)',marginBottom:28}}>抽一张卡 随机游戏赢水晶</span>
          ...
          {phase === 'intro' && (
            <div onClick={drawCard} style={{
              ...
              '--gc':'rgba(160,120,200,0.35)',marginBottom:20,touchAction:'manipulation',
            } as any}>
          ...
          {phase === 'reveal' && gamePicked && (
            <div style={{alignItems:'center',gap:14,display:'flex',flexDirection:'column'}}>
              <div style={{
                ...
                <span style={{fontSize:52,marginBottom:6}}>{gamePicked.icon}</span>
                <span style={{fontSize:17,fontWeight:700,color:'#f0e8d0',marginBottom:2,
                  animation:`title-glow 0.8s ease-in-out infinite`,
                  '--tc':gamePicked.color} as any}>{gamePicked.name}</span>
                <span style={{fontSize:10,color:'rgba(255,255,255,0.35)'}}>{gamePicked.desc}</span>
              </div>
              {/* 加载粒子 */}
              <div style={{display:'flex',flexDirection:'row',gap:6}}>
                {[0,1,2].map(i=>(
                  <div key={i} style={{width:6,height:6,borderRadius:3,background:gamePicked.color,
          ...
          {/* 星星粒子 */}
          {stars.map(s=>(
            <div key={s.id} style={{position:'absolute',left:'50%',top:'45%',width:4,height:4,borderRadius:2,
      ...
      {phase === 'result' && gemWon && (
        <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:30}}>
          <div style={{background:'#FAF6ED',borderRadius:20,padding:'24px 28px',alignItems:'center',gap:8,display:'flex',flexDirection:'column',maxWidth:280}}>
            {gemWon.type==='scrap' ? (
              <>
                <div style={{width:48,height:48,borderRadius:10,background:'#c8b8a8',display:'flex',alignItems:'center',justifyContent:'center'}}>
                  <span style={{fontSize:24,opacity:0.6}}>💎</span>
                </div>
                <span style={{fontSize:15,fontWeight:700,color:'#928370'}}>获得碎石废料</span>
                <span style={{fontSize:10,color:'#928370',textAlign:'center'}}>表现不太好，下次加油</span>
              </>
            ) : (
              <>
            ...
            {!collected ? (
              <div onClick={handleCollect} style={{marginTop:10,padding:'10px 36px',borderRadius:25,
                background:'linear-gradient(135deg,#d4a574,#c4956a)',cursor:'pointer',
                boxShadow:'0 3px 10px rgba(212,165,116,0.3)',touchAction:'manipulation'}}>
                <span style={{fontSize:13,fontWeight:700,color:'#fff'}}>放入背包</span>
              </div>
            ) : (
              <>
                <span style={{fontSize:11,color:'#9db9a5',fontWeight:600}}>已放入背包</span>
                <div style={{display:'flex',flexDirection:'row',gap:10,marginTop:6}}>
                  <div onClick={goHome} style={{padding:'8px 20px',borderRadius:20,
                    border:'1px solid #d4c0a4',cursor:'pointer',touchAction:'manipulation'}}>
                    <span style={{fontSize:12,color:'#4A382C'}}>返回首页</span>
                  </div>
                  <div onClick={playAgain} style={{padding:'8px 20px',borderRadius:20,
                    background:'linear-gradient(135deg,#d4a574,#c4956a)',cursor:'pointer',touchAction:'manipulation'}}>
